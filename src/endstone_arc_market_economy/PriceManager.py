...
MAIN_PATH = 'plugins/ARCMarketEconomy'
...
class PriceManager:
    ...
    def _load_config(self):
        ...
        try:
            ...
            self._safe_log('info', f"[ARCMarketEconomy] Loaded official prices: {count} items from {self.config_path}")
            if count == 0:
                self._safe_log(
                    'warning',
                    f"[ARCMarketEconomy] official_prices.yml exists but 0 items parsed "
            ...
            self._safe_log(
                'error',
                f"[ARCMarketEconomy] Failed to load official_prices.yml: {e}\n{traceback.format_exc()}"
            )
...
"""
        try:
            ...
            self._safe_log('info', f"[ARCMarketEconomy] Created default official_prices.yml ({len(self.official_prices)} items)")
        except Exception as e:
            self._safe_log('error', f"[ARCMarketEconomy] Failed to create default config: {e}")
    ...
    def _parse_config(self, content: str):
        ...
        self._safe_log(
            'info',
            f"[ARCMarketEconomy] Parsed {len(self.official_prices)} official prices "
    ...
    def record_trade_volume(self, item_type: str, shop_type: str, quantity: int, db_manager, total_amount: float = 0):
        ...
        try:
            ...
            self._safe_log('error', f"[ARCMarketEconomy] Record trade volume error: {e}")
    ...
    def load_price_adjustments_from_db(self, db_manager):
        ...
        try:
            ...
            if rows:
                ...
                self._safe_log('info', f"[ARCMarketEconomy] Loaded {len(rows)} price adjustments from DB")
        except Exception as e:
            self._safe_log('error', f"[ARCMarketEconomy] Load price adjustments from DB error: {e}")
    ...
    def _apply_daily_fluctuation(self, db_manager):
        """应用每日随机波动：价目表内全部物品各自随机一次。
        daily_fluctuation_item_count 已废弃（兼容保留配置键，不再限制数量）。
        """
        ...
        applied = 0
        for item_type in priced_items:
            fluctuation = round(random.uniform(min_percent, max_percent), 1)
            if item_type not in self.price_adjustments:
                ...
                self.price_adjustments[item_type]['last_updated'] = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        ...
        self._safe_log(
            'info',
            f"[ARCMarketEconomy] Applied daily fluctuation to ALL {applied} priced items "
            f"(range {min_percent}% .. {max_percent}%)"
        )
    ...
    def cleanup_old_trade_volumes(self, db_manager, days: int = 7):
        ...
        try:
            ...
            self._safe_log('error', f"[ARCMarketEconomy] Cleanup old trade volumes error: {e}")
    ...
    def reset_all_adjustments(self, db_manager):
        ...
        try:
            ...
            self._safe_log('info', "[ARCMarketEconomy] All price adjustments reset")
        except Exception as e:
            self._safe_log('error', f"[ARCMarketEconomy] Reset price adjustments error: {e}")
